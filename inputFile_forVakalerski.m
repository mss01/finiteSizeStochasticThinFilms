...
switch filmConfiguration
    ...
    case 'finiteSizedNonFlatFilms'
        ...
        R_film = [5 10 15 20 25 30 35 40 50 60 65 70 75 80 85 90 100 115 150 200 300 400 500 600 700 800 900 1000];   % radius of the film
%         R_film = [70 90 150 800];
        R_f = R_film.*10^-6;              % in m
        h0_init = 20e-9;                 % initial film height in m
        A_vw = 5e-20;                  % Hamaker constant
        gam = 0.072;                      % surface tension
        Rc = 90e-6;                      % radius of capillary
        visc = 0.00089;                   % viscosity
        t_cr_dimensional = [2 3 4 5].*10^-3;     % time resolution (in sec) 
        res_limit_dimensional = 5e-6;    % spatial resolution (in m)
        ...
        h_drain_start = 20e-9/h0_init;                 % 100nm as mentioned in Wasan & Malhotra (1987)
        h_drain_end = 1e-9/h0_init;                    % 25 nm as mentioned in Wasan & Malhotra (1987)
        hJoyeStart = 0.9;                                 % determine when to start measuring thinning rates
        hJoyeEnd = 0.627*kappa^(-2/7);                  % where to end
        h_critical_start = 0.627*kappa^(-2/7)*1.0;      % this is when the film thinning velocity starts becoming nearly constant
...
%         ctimestep = 3;                              % exponent used in deciding deltaT = deltaX^c --> although c = 2.75 suffices, but a higher temp resolution enables more time stamps
        ...
        animationSkip = 10;                 % save animation every these many time steps
