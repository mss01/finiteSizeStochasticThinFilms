...
h_drain_start = 100e-9/h0_init;                 % 100nm 
h_drain_end = 25e-9/h0_init;                    % 25 nm
h_critical_start = 0.12;                        % this is when the film thinning velocity starts becoming nearly constant
h_critical_end = 0.08;                           % this is when the film thinning velocity ends becoming nearly constant
...
[h x] = initialProfile(kappa,L_flat(1),L_curv,transitionLength,deltaX(1));
plot(x,h,'o')
ylim([0.95 5])
...
for i = 1:length(R_film)
    ...
    cd(run_mainFiles{i})
    main(R_f(i), h0_init, A_vw, gam, Rc, visc, L_flat(i), N_flat(i), deltaX(i), deltaT(i), transitionLength, h_drain_start, h_drain_end, h_critical_start,...
