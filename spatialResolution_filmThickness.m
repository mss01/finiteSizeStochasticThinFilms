...
%     x = x';
    ...
    for i = 1:length(t)-1;
        ...
        if mod(round(res_limit/deltaX),2) ~= 0
%             indexwa = idx_leftMin(i)
%             aa = x_le(idx_leftMin(i))
%             ab = x_le(round((res_limit/2)/deltaX))
%             ac = idx_leftMin(i) + round((res_limit/2)/deltaX)
%             ad = idx_leftMin(i) - round((res_limit/2)/deltaX)
%             ae = idx_leftMin(i) + round(res_limit/deltaX) + 1
...
%% calculate region around the left dimple

%     x_le = x(x<0);
%     x_dummy = ones(size(x_le));
%     for i = 1:length(t)-1;
%         h_l = c(:,i);
%         h_le(:,i) = h_l(x<0);
%         [val_leftMin(i) idx_leftMin(i)] = min(h_le(:,i));
%         if mod(round(res_limit/deltaX),2) ~= 0
%             if ( x_le(idx_leftMin(i))  + x_le(round((res_limit/2)/deltaX)) ) < 0;
%                 idx_interest1 = x_dummy(x_le < (x_le(idx_leftMin(i)  + round((res_limit/2)/deltaX))) & x_le >= (x_le(idx_leftMin(i)  - round((res_limit/2)/deltaX))))
%                 h_left(:,i)  = h_le((x_le < (x_le(idx_leftMin(i)  + round((res_limit/2)/deltaX))) & x_le >= (x_le(idx_leftMin(i)  - round((res_limit/2)/deltaX)))),i);
%                 x_left(:,i)  = x_le( x_le < (x_le(idx_leftMin(i)  + round((res_limit/2)/deltaX))) & x_le >= (x_le(idx_leftMin(i)  - round((res_limit/2)/deltaX))))';
%             else
%                 h_left(:,i)  = h_le((x_le < (x_le(idx_leftMin(i)))  & x_le >= (x_le(idx_leftMin(i)  - round(res_limit/deltaX) + 1))),i);
